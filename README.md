...
```js
// with opts
const router = flare({ prefetch: 'visible', log: false, pageTransitions: false });
...
```js
const router = flare({ prefetch: 'visible' });
```
...
You can deploy Flare.js to [Vercel](http://vercel.com/) as follows:
